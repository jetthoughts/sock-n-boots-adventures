...
(function($) {
    var Flip = function(canvas, width, height, images, opts) {
        ...
        var obj = this, el = canvas.prev(), index = 0, init = false, background = opts.background, cornersTop = opts.cornersTop, gradientColors = opts.gradientColors || ['#D1D1D1', '#F5D7D5', '#F2E2E1'], curlSize = opts.curlSize || 0.1, scale = opts.scale, patterns = [], canvas2 = canvas.clone(), ctx2 = $.browser.msie ? null : canvas2[0].getContext("2d"), canvas = $.browser.msie ? $(G_vmlCanvasManager.initElement(canvas[0])) : canvas, ctx = canvas[0].getContext("2d"), loaded = 0;
        var touchPnt = {x:0, y:0};
        var images = images.each(
            function(i) {
                ...
                img.onload = function() {
                    var r = 1;
                    if (scale != "noresize") {
                        var rx = width / this.width, ry = height / this.height;
                        if (scale == "fit")
        ...
        var
            width = width, height = height, mX = width, mY = height, basemX = mX * (1 - curlSize), basemY = mY * curlSize, sideLeft = false, off = $.browser.msie ? canvas.offset() : null, onCorner = false, curlDuration = 400, curling = false, animationTimer, startDate, flipDuration = 200, flipping = false, baseFlipX, baseFlipY, lastmX, lastmY, inCanvas = true, mousedown = true, disabled = false, dragging = false;
        ...
        c.mousemove(
            ...
            function(e) {
                ...
                if (touchPnt.x == e.pageX && touchPnt.y == e.pageY) {
                    el.trigger("flip.click", touchPnt);
                }
                else if (onCorner && !flipping && (sideLeft && (index > 0) || (!sideLeft && index < images.length - 1) )) {
                    _startFlip();
                }
